...
flowchart LR
  ...
  Trial --> Journey[Профиль → AI-подбор → проверка базы<br/>→ почта → личный адрес → кампания → Inbox]
  Journey --> Finish[Финал онбординга<br/>рекомендуем Базовый]
  Finish --> Choice{Перейти к рабочему<br/>тарифу?}
  Choice -->|да, без автоматической оплаты| Plans[Внутренний экран тарифов<br/>Базовый рекомендован]
  Plans --> Renewal{Включить автопродление?}
  ...
  Frozen --> Payment
  Choice -->|исследовать Smailee| Trial
  Trial --> Reminder[Компактное предложение на Главной<br/>+ контекстный paywall по лимиту]
```
