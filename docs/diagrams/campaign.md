...
flowchart TD
  ...
  Who --> Context[Загрузить опубликованный профиль организации<br/>и релевантные данные выбранного сегмента]
  Context --> Multi{Сегментов больше одного?}
  ...
  ManualText --> Edit
  Edit --> Preview[Сгенерировать до 5 финальных персональных писем<br/>список контактов → полный текст<br/>показанные версии сохраняются в Message]
  Preview --> Chain[Цепочка follow-up:<br/>сколько писем и с каким интервалом]
  Chain --> Track[Open Rate по желанию:<br/>минимальный HTML только с пикселем открытия]
  Track --> Schedule[Дата и время старта<br/>при необходимости «Отправлять в любое время»]
  Schedule --> Access{Демо или оплаченный<br/>доступ ещё действует?}
  ...
  Quota -->|да| Materialize[«Создать и запустить»<br/>на каждый сегмент — своя кампания<br/>письма встают в очередь]
  ...
  Materialize -.-> InboxQueue[Готовое письмо видно в Inbox<br/>с причиной ожидания]
  InboxQueue -->|редактировать| InboxQueue
  InboxQueue -->|отменить| Cancelled([Письмо исключено из отправки<br/>и общего количества кампании])

  Warm{Есть прогретый ящик?}
  Warm -->|нет| WaitWarm
  Warm -->|да| Anytime{Разрешено отправлять<br/>в любое время?}
  Anytime -->|да| RuntimeAccess
  Anytime -->|нет| Window{Рабочее окно?<br/>Пн–Пт, 9–19 МСК}
  ...
  Sent -.-> Analytics[Раздел «Аналитика»:<br/>одна кнопка фильтров по датам,<br/>нескольким кампаниям и сегментам]
  Analytics --> OpenTracking{Во всех выбранных кампаниях<br/>включён трекинг открытий?}
  OpenTracking -->|да| FullFunnel[Отправлено — входная база;<br/>доставлено → открыли → ответили → тёплые<br/>с конверсией между этапами]
  OpenTracking -->|нет| SafeFunnel[Этап открытий скрыт;<br/>доставлено → ответили → тёплые<br/>без искажения конверсии]
  ...
  Answered -->|да| Thread[Ответ склеивается в тред<br/>и появляется в отдельном Inbox]
  Thread -.-> InboxFlow[Компактный список → выбранная переписка;<br/>фильтры по кампании, ящику и состоянию<br/>см. inbox.md]
  ...
  Trigger -->|нет| Retrieval[Найти в профиле и проиндексированных страницах<br/>3–5 фактов по вопросу клиента]
  Retrieval --> Dialog[ИИ пишет ответ по подтверждённому контексту;<br/>если данных нет — не выдумывает]
