...
Сформированное письмо появляется здесь ещё до SMTP. Плашка «Пока не отправили» объясняет ожидание: персонализация, выбранное время старта, готовность ящика, рабочее окно или очередь. Готовое `PENDING`-письмо можно отредактировать или отменить; захваченное воркером `QUEUED` уже недоступно для изменения. `CANCELLED` не показывается и не входит в объём кампании.
