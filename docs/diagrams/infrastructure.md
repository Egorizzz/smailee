...
Провижининг доменов, DNS и почтовых ящиков происходит вне Smailee. Smailee подключается к готовым ящикам и проверяет SMTP/IMAP реальным входом. Готовые профили подключения есть для Яндекс Почты (включая `yandex.ru` и Яндекс 360), Gmail / Google Workspace и Почты Mail; для каждого используются стандартные SSL-подключения SMTP и IMAP и отдельный пароль приложения. В онбординге для Яндекса показывается сокращённый путь: включить IMAP и пароли приложений в настройках Почты, создать пароль для «Почты» и вставить его в Smailee. Полная инструкция по подготовке корпоративного домена и сотрудников остаётся в разделе инфраструктуры.
