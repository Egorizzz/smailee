...
export function CreateClientForm({
  ...
  const [delivery, setDelivery] = useState<"email" | "copy">("email");
  ...
  return (
    <form action={formAction} className="grid gap-4 sm:grid-cols-2">
      ...
      {selectedSuggestion ? (
        ...
        <>
          <input name="email" type="email" required placeholder="Email клиента" className="input" value={manualEmail} onChange={(event) => setManualEmail(event.target.value)} />
      ...
      <fieldset className="sm:col-span-2">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          <label className={`cursor-pointer rounded-xl border p-4 ${delivery === "email" ? "border-mint-400 bg-mint-50" : "border-line bg-white"}`}>
            <input type="radio" name="delivery" value="email" checked={delivery === "email"} onChange={() => setDelivery("email")} />
      ...
      <p className="rounded-lg bg-indigo-50 px-3 py-2 text-xs leading-5 text-indigo-800 sm:col-span-2">
        Клиент войдёт по одноразовой ссылке без пароля. Ссылка из письма подтвердит email; ссылка для мессенджера — только вход.
      </p>
      ...
      <button disabled={pending || !source || !currentEmail} className="rounded-lg brand-gradient-vivid px-5 py-2.5 text-sm font-semibold text-white glow disabled:opacity-60 sm:col-span-2">
