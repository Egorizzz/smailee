    ...
    <div className="mx-auto max-w-4xl">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Link
            href="/app/campaigns/series/new"
            className="rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2.5 text-sm font-semibold text-indigo-700"
          >
            + Серия контент-маркетинга
          </Link>
          <Link
      ...
      <div className="mt-6 space-y-3">
        ...
        {campaigns.map((c) => (
          <Link
            key={c.id}
            href={c.type === "SERIES" ? `/app/campaigns/series/${c.id}` : `/app/campaigns/${c.id}`}
          ...
          >
            <div>
              ...
              <div className="mt-1 text-sm text-ink-500">
                {c.type === "SERIES" ? c.seriesTopic : c.subject} · {c._count.messages} писем
              </div>
            </div>
