...
export default async function InitialAccessPage({
  ...
  if (access && access.type !== "INITIAL_ACCESS") redirect("/login");
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-5 py-12">
      <div className="mb-8 text-center">
        ...
        <h1 className="mt-6 text-2xl font-bold text-slate-900">Доступ в Smailee готов</h1>
      ...
      {expired ? (
        <a href="/login" className="rounded-lg brand-gradient px-4 py-3 text-center text-sm font-semibold text-white">Войти по логину и паролю</a>
      ) : (
        <div className="rounded-2xl border border-line bg-white p-5 shadow-sm">
          <label className="text-sm font-medium text-slate-900">Логин</label>
          <div className="mt-2 rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink-700">
            {access.user.emailPending ? access.user.login : access.user.email}
          </div>
          ...
          <p className="mt-3 text-center text-xs leading-5 text-ink-500">Ссылка одноразовая. После входа постоянные данные можно настроить в разделе «Вход и безопасность».</p>
        </div>
      )}
    </div>
  );
